...
5. Add a `Remap (Operator Math Remap)`, connect the `Perlin Curl Noise 2D > Noise` output to its `input` input. Set `Old Range Min: -3`, `Old Range Max: 3`, `New Range Min: -1`, and `New Range Max: 1`. Replace the `X` and `Y` input of `Conform to Sphere` with the `X` and `Y` output from `Remap (Vector2)`.
